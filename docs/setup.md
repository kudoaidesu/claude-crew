...
`~/Library/LaunchAgents/com.pocket-cc.web.plist` を作成:
...
<dict>
    <key>Label</key>
    <string>com.pocket-cc.web</string>
    ...
    <string>/path/to/pocket-cc</string>
    ...
    <string>/path/to/pocket-cc/data/logs/web.log</string>
    <key>StandardErrorPath</key>
    <string>/path/to/pocket-cc/data/logs/web.err.log</string>
</dict>
</plist>
```

> `/path/to/pocket-cc` を実際のパスに置き換えること。

```bash
# サービス登録・起動
launchctl load ~/Library/LaunchAgents/com.pocket-cc.web.plist

# サービス停止
launchctl unload ~/Library/LaunchAgents/com.pocket-cc.web.plist
...
`/etc/systemd/system/pocket-cc-web.service` を作成:

```ini
[Unit]
Description=pocket-cc Web UI
After=network.target

[Service]
Type=simple
User=your-username
WorkingDirectory=/path/to/pocket-cc
...
```bash
# サービス有効化・起動
sudo systemctl enable pocket-cc-web
sudo systemctl start pocket-cc-web

# 状態確認
sudo systemctl status pocket-cc-web

# ログ確認
journalctl -u pocket-cc-web -f
```
